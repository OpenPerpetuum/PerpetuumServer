...
namespace Perpetuum.Services.TechTree
{
    public class TechTreePointsHandler
    {
        ...
        {
            var record = Db.Query().CommandText("select top 1 id,amount from techtreepoints (UPDLOCK) where owner = @owner and pointType = @pointType")
        ...
        {
            return Db.Query().CommandText("select pointType,amount from techtreepoints (UPDLOCK) where owner = @owner").SetParameter("@owner",_owner).Execute().Select(r => new Points(r.GetValue<TechTreePointType>(0), r.GetValue<int>(1)));
        }
    }
}